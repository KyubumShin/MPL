...
if matches.length > 0:
  ...
  Bash("echo '" + JSON.stringify(metric) + "' >> .mpl/mpl/pattern-metrics.jsonl")
...
Replaces the old Zero-Test Block. Decomposer emits `test_agent_required: bool` per phase; the `hooks/mpl-require-test-agent.mjs` PostToolUse hook **structurally enforces** dispatch (outputs `decision: "block"` on violations). The Zero-Test Block below remains as an additional safety net for the case where tests exist but some required phases were dispatched without a corresponding test-agent call.

```
decomposition = Read(".mpl/mpl/decomposition.yaml")
required_phases = decomposition.phases.filter(p => p.test_agent_required != false)
dispatched = state.test_agent_dispatched or {}
override = Read(".mpl/config/test-agent-override.json") or {}
...
if missing.length > 0:
  ...
  for phase_id in missing:
    ...
    Task(subagent_type="mpl-test-agent", model="sonnet",
      prompt="Write independent tests for phase {phase.id} ({phase.phase_domain}).
      AD-0004 contract: you are NOT the phase-runner. Treat the impact files as
      a black box; derive tests from the interface_contract, not from reading
      implementation details.
      Implemented files: {phase.created + phase.modified files}
      Interface contracts: {phase.interface_contract}
      Probing hints (HA-03): {phase.probing_hints or []}
...
# Legacy zero-test fallback (pre-AD-0007 decompositions without test_agent_required
# field). Retained for backwards-compat; removed once all historic pipelines are
# re-decomposed under v0.15.1+.
test_count = count total tests from test runner output
legacy_mandatory = decomposition.phases.filter(p =>
  p.test_agent_required == null AND p.phase_domain in ["ui", "api", "algorithm", "db", "ai"]
)
if test_count == 0 AND legacy_mandatory.length > 0:
  Hard 2 = FAIL
  announce: "[MPL] Legacy Zero-Test fallback: forcing test-agent for {legacy_mandatory.length} phases without test_agent_required field."
  for phase in legacy_mandatory:
