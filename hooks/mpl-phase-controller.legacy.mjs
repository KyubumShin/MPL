...
async function main(prefetchedInput) {
  const input = prefetchedInput !== undefined ? prefetchedInput : await readStdin();
...
// Exported entry-point used by the Move #9 wrapper to forward stdin
// payloads it has already read. When invoked directly as a script (legacy
// fallback path), the bottom-of-file main().catch() at file scope still
// runs main() with no argument, reading stdin itself.
export async function runLegacyMain(stdinPayload) {
  await main(stdinPayload);
}

// Only auto-execute when this file is the entry script (legacy fallback).
const _isLegacyMain = import.meta.url === pathToFileURL(process.argv[1] || '').href;
if (_isLegacyMain) {
  main().catch(() => {
    console.log(JSON.stringify({ continue: true, suppressOutput: true }));
  });
