...
// v2 Stage A: read-side state utilities now live in `./state/reader.mjs`.
// The facade re-imports every symbol previously defined inline here so all
// existing import sites (40 isMplActive / 30 readState / 1 checkConvergence /
// 1 migrateLegacyExecutionState / 1 initState, plus the CURRENT_SCHEMA_VERSION
// / MAX_AMBIGUITY_HISTORY / LEGACY_EXECUTION_STATE_PATH constants) keep
// working without edits. `applyMigrationChain` and `readPersistedSchemaVersion`
// stay INTERNAL — imported for the writer's H8 fail-closed guard but not
// re-exported.
import {
  readState,
  isMplActive,
