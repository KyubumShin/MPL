/**
 * MPL v2 L1 state writer — write-side access to `.mpl/state.json`.
 *
 * Stage A Move #2: the write-side state utilities are lifted verbatim from
 * `hooks/lib/mpl-state.mjs` so the new `hooks/lib/state/` layer owns the
 * writer side without disturbing any caller. `hooks/lib/mpl-state.mjs`
 * re-exports every public symbol below under the same name, so all existing
 * import sites (writeState ×many, initState ×1, cleanPipelineScope ×1,
 * UnsupportedSchemaVersionError) keep working without edits.
 *
 * Exported symbols:
 *   Functions:
 *     - writeState(cwd, patch)              (public)
 *     - initState(cwd, featureName, mode)   (public)
 *     - cleanPipelineScope(cwd)             (public)
 *
 *   Classes:
 *     - UnsupportedSchemaVersionError       (public)
 *
 * Contract:
 *   - Atomic write via temp + rename (C2).
 *   - H8 fail-closed guard: refuses to overwrite a fresher (higher
 *     schema_version) on-disk state with a downgraded shape.
 *   - I5 lockstep + revert-on-violation between fix_loop_count and
 *     fix_loop_history (G5 / #114, PR #133 review #3-#4).
 *   - RUNBOOK row append after rename (G2 / #113, non-blocking).
 *   - Ring-buffer caps: ambiguity_history (10), phase_scheduler_history
 *     (50), worktree_pool_history (50).
