...
PM 기능은 별도 Step이 아니라, **기존 Step 1(PP Interview)을 확장하여 PP + 요구사항 인터뷰로 통합**한다. Triage의 기존 `interview_depth`(light/full) 필드가 PM 기능의 활성화 범위를 자연스럽게 제어한다. (F-35: `skip` 옵션은 제거됨 — 인터뷰는 항상 최소 light 수준으로 실행)
...
Step 0: Triage (기존)
  ...
  ├── interview_depth 결정 (light / full)  ← PM 범위도 이것으로 제어 (F-35: skip 제거)
...
|-------|----------|---------------|-------------|-----------|
| light (density ≥ 8) | Round 1-2 + PP 직접 추출 | Uncertainty Scan (HIGH만 타겟 질문) | HIGH 항목 한정 (0~3개) | 없음 |
| light (density 4-7) | Round 1-2 (What + What NOT) | 경량 구조화 (US + AC) | 명확화 + 가정 탐색만 | 없음 |
| full  | Round 1-4 전체 | JUSF 전체 | 6유형 전체 | 3+ 옵션 + 매트릭스 |

> **NOTE (F-35)**: `skip` 옵션은 제거되었다. 전체 스펙 구현 보장을 위해 인터뷰는 항상 실행된다.
> 고밀도 프롬프트(density ≥ 8)도 최소 Round 1-2 인터뷰를 거치며, Uncertainty Scan으로 보강된다.
...
| **불확실성 묵살** | 상세 프롬프트에서 암묵적 가정/모호한 기준을 놓침 | 고밀도 프롬프트(density ≥ 8)에서도 Round 1-2 인터뷰 + Uncertainty Scan으로 HIGH 불확실성 사전 식별 (F-35) |
...
|-------|-----------|-------------------|
| light (density ≥ 8) | 3개 | Round 1-2 + Uncertainty Scan HIGH 항목 3개 후 → 남은 항목 있으면 Continue Gate |
| light (density 4-7) | 4개 | PP Round 1-2 + 소크라틱 2개 후 → 남은 불확실성 있으면 Continue Gate |
...
#### light 모드 — 고밀도 (density ≥ 8, PP Round 1-2 + Uncertainty Scan)

> F-35: 기존 `skip` 모드를 대체. 인터뷰는 항상 최소 Round 1-2를 실행한다.

```
Round 1-2 인터뷰 (What + What NOT)
  → PP 후보 추출 + 프롬프트 직접 PP 추출
  → Uncertainty Scan (9차원: 3축 × 3 + 축 간 교차)
...
#### light 모드 — 표준 (density 4-7, PP Round 1-2 + 경량 PM)
```
[Context Loading]
...
| `interview_depth` | Y | light / full (F-35: skip 제거) |
...
| 차원 | light (density ≥ 8) | light (density 4-7) | full |
|------|---------------------|---------------------|------|
| **PP Rounds** | Round 1-2 + PP 직접 추출 | Round 1-2 | Round 1-4 전체 |
| **Uncertainty Scan** | **✅ Round 1-2 후 9차원 불확실성 검사** | PP 라운드에서 자연 해소 | PP 라운드에서 자연 해소 |
| **Job Definition** | PP Round 1에서 자동 도출 | PP Round 1에서 자동 도출 | Full JTBD |
| **소크라틱 질문** | **HIGH 항목 한정 (0~3개)** | 명확화 + 가정 탐색 (2유형) | 6유형 전체 |
| **User Stories** | 경량 구조화 | 경량 구조화 | 전체 작성 |
| **Gherkin AC** | 핵심 AC만 | 핵심 AC만 | 전체 + Edge Cases |
| **솔루션 옵션** | 없음 | 없음 | 3개+ |
| **PP 후보** | Round 1-2에서 추출 | Round 1-2에서 추출 | 전체 라운드에서 추출 + 확정 |
| **MoSCoW** | 암시적 (Must만) | 암시적 (Must만) | 명시적 분류 |
| **증거 태깅** | 🟢/🔴만 | 🟢/🔴만 | 🟢/🟡/🔴 전체 |
| **다관점 검토** | 없음 | 없음 | 3관점 전체 |
| **예상 토큰** | ~1.5-2.5K | ~2K | ~5K |
| **모델** | Opus | Sonnet | Opus |

> **NOTE (F-35)**: `skip` 행이 제거되고 `light (density ≥ 8)` 행으로 대체되었다. 인터뷰는 항상 실행된다.
...
interview_depth =
  if information_density >= 8:
    "light"       # 요청이 상세해도 인터뷰 필수 → Round 1-2 + Uncertainty Scan (F-35)
...
> **인터뷰 필수화 (F-35)**: `skip` 옵션은 제거되었다. 전체 스펙 구현을 위해 인터뷰는 항상 최소 light 수준으로 실행된다. "정보 밀도가 높다"는 것은 양이 많다는 의미이지, 모든 것이 명확하다는 의미가 아니다. 고밀도 프롬프트(density ≥ 8)에서도 Round 1-2 인터뷰 후 Uncertainty Scan(9차원 불확실성 검사)을 수행하고, HIGH 불확실성이 발견되면 해당 항목에 대해서만 타겟 소크라틱 질문을 수행한다(최대 3개). 상세 프로토콜은 `agents/mpl-interviewer.md`의 `<Uncertainty_Scan>` 섹션 참조.

### 8.3 Pipeline Tier와의 상호작용

| Pipeline Tier | interview_depth 경향 | PM 동작 |
|---------------|---------------------|---------|
| **Frugal** (< 0.3) | light (+ Uncertainty Scan) | 경량 요구사항 확인 (F-35: 인터뷰 필수) |
