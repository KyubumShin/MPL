...
The top-level `install.sh` supports curl-based gitless installation by downloading a GitHub source archive into a persistent source root (default `~/.mpl/install/source/mpl`) and writing `.mpl-install-manifest`. Claude registers that persistent source directly and accepts `--scope user|project|local|ask` for the native Claude plugin scope. `--scope ask` reads from `/dev/tty`, so it works with `curl | bash` while keeping Enter as the `user` default; use an explicit scope in non-interactive automation. Codex stages `./plugins/mpl` from the manifest so local state and build artifacts are not copied into Codex plugin cache.
