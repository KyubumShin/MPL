...
The top-level `install.sh` supports curl-based gitless installation by downloading a GitHub source archive into a persistent source root (default `~/.mpl/install/source/mpl`) and writing `.mpl-install-manifest`. Claude registers that persistent source directly. Codex stages `./plugins/mpl` from the manifest so local state and build artifacts are not copied into Codex plugin cache.
