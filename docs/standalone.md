...
|---------|------------------|------------|
| Claude Code | `install.sh` bootstrap plus `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` | `.mcp.json` |
| Codex CLI | `install.sh` bootstrap plus `.codex-plugin/plugin.json`; wrapper marketplace generated by `install/codex.sh` | `.codex-plugin/.mcp.json` |

The top-level `install.sh` supports curl-based gitless installation by downloading a GitHub source archive into a persistent source root (default `~/.mpl/install/source/mpl`) and writing `.mpl-install-manifest`. Claude registers that persistent source directly and accepts `--scope user|project|local|ask` for the native Claude plugin scope. `--scope ask` reads from `/dev/tty`, so it works with `curl | bash` when a real interactive TTY is available, keeps Enter as the `user` default, and exits with an error when no TTY is available. Use an explicit scope in CI or other headless automation. Codex stages `./plugins/mpl` from the manifest so local state and build artifacts are not copied into Codex plugin cache.

Both MCP configs use `tools/mpl-mcp-server-launcher.mjs`. The Codex config uses plugin-root relative `node ./tools/mpl-mcp-server-launcher.mjs` with `cwd: "."`; the Claude config uses `${CLAUDE_PLUGIN_ROOT}` because Claude MCP commands resolve from the user workspace unless the plugin root is explicit. The launcher builds `mcp-server/` on first use without writing to MCP stdout.
