...
| Codex marketplace | None | `install/codex.sh` generates a Codex wrapper marketplace whose `.agents/plugins/marketplace.json` exposes `mpl` via `./plugins/mpl` | Install metadata | Avoids forcing the repo root into Codex marketplace layout, which conflicts with Claude marketplace expectations |
