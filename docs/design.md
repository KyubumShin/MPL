...
| `mpl-decomposition-postprocess` | PostToolUse: Edit/Write/MultiEdit | Regenerate `.mpl/mpl/decomposition-derived.json` immediately after `decomposition.yaml` changes. | v0.18 Phase 5 diet |
...
| Blocked hook atomicity | `session_status: blocked_hook` could remain after reason/instruction fields were cleared | I11 requires hook id, phase, artifact, stable code, reason, resume instruction, retry context, and timestamp when blocked; `writeState` clears the full envelope when status clears | State writer + invariant | Prevents silent hook blocks from turning into unactionable zombie state |
...
| Visible hook block state | AD-0007 blocks were returned only as hook output, so harness/UI could see a long silence with `pause_reason:null` | Covered hooks record `session_status: blocked_hook` plus hook id, phase, artifact, code, reason, resume instruction, retry context, and clear it when evidence/override appears | Hook + state enum | Prevents quality guard blocks from presenting as silent deadlocks |
...
| Change | Before | After | Type | Rationale |
|--------|--------|-------|------|-----------|
| Bootstrap installer | Git clone or local checkout required | `install.sh` can be downloaded with curl and installs Claude, Codex, or both from a GitHub source archive | Install UX | Supports gitless install while keeping a persistent source root for runtime marketplaces |
...
| Codex marketplace | None | `install/codex.sh` generates a Codex wrapper marketplace whose `.agents/plugins/marketplace.json` exposes a clean staged `mpl` root via `./plugins/mpl` | Install metadata | Avoids forcing the repo root into Codex marketplace layout and prevents local state/build artifacts from being copied into Codex plugin cache |
| MCP config | Claude-only `.mcp.json` used prebuilt `mcp-server/dist/index.js` | Claude and Codex both use `tools/mpl-mcp-server-launcher.mjs`; Claude via `${CLAUDE_PLUGIN_ROOT}`, Codex via plugin-root relative `cwd: "."` | MCP config | Lets archive installs bootstrap `mcp-server/` on first use without requiring committed build output |
...
| Goal source naming | Goal Contract examples hardcoded `source.codex_goal` | Canonical `source.runtime` + `source.runtime_goal`, with `codex_goal`/`claude_goal` compatibility aliases | Schema + hook | Makes the frozen goal read naturally in Claude and Codex sessions without breaking existing contracts |
| Version alignment | Version checklist covered Claude + MCP only | Tests and bump checklist cover Claude, Codex, the installer-generated marketplace, MCP package, and README/docs headings | Test + process | Prevents stale version drift across dual-runtime metadata |

Codex staging is intentionally a clean manifest-based copy rather than a symlink to the source. For git checkouts, the manifest is `git ls-files`; for curl/archive installs, `install.sh` writes `.mpl-install-manifest` from the downloaded archive. This keeps `.git`, `.mpl`, `.pr-review-state`, `.claude`, `node_modules`, and stale `mcp-server/dist` output out of Codex plugin cache. The tradeoff is explicit refresh: after updating MPL, rerun `install.sh` or `install/codex.sh` from a refreshed source so Codex sees the updated staged root.

**Affected files:** `install.sh`, `.mcp.json`, `.codex-plugin/plugin.json`, `.codex-plugin/.mcp.json`, `install/claude.sh`, `install/codex.sh`, `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `tools/mpl-mcp-server-launcher.mjs`, `mcp-server/package.json`, `mcp-server/package-lock.json`, `README.md`, `README_ko.md`, `commands/mpl-run-phase0.md`, `docs/schemas/goal-contract.md`, `docs/standalone.md`, `docs/config-schema.md`, `docs/roadmap/overview.md`, `skills/mpl-setup/SKILL.md`, `skills/mpl-doctor/SKILL.md`, `skills/mpl-version-bump/SKILL.md`, `agents/mpl-doctor.md`, `hooks/lib/mpl-goal-contract.mjs`, `hooks/__tests__/mpl-goal-contract.test.mjs`, `hooks/__tests__/mpl-install-metadata.test.mjs`.
...
| test_agent_dispatched field | not tracked | `state.test_agent_dispatched[phase_id] = {timestamp, prompt_len, response_len, valid_json, verdict, tests_*, command_exit_codes, ...}` populated on Task(mpl-test-agent) completion | Schema + hook | AD-0004 empirical gap closure; exp21 hardening requires PASS evidence, not dispatch existence |
