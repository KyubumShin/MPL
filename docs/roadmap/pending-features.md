...
| T-02 | ~~Cross-Model Review~~ | `/codex` | ~~🔴~~ **Dropped** — Always-On Judge 흡수 (codex-plugin-cc) | Medium (API integration) | ~5-10K/review |
...
| T-05 | ~~Design Contract~~ | `/design-consultation`, `/design-review` | ~~🟡~~ **Dropped** — Seed contract_snippet 대체 | Medium | ~8-12K |
| T-06 | Doc Sync | `/document-release` | 🟡 **v1.0.1** | Low | ~3-5K |
| T-07 | Premise Challenge Mode | `/office-hours` | 🟢 Optional | Low | ~2K |
| T-08 | Trend Retro | `/retro` | 🟢 Optional | Low | ~3K |
| T-09 | ~~Performance Gate~~ | `/benchmark` | ~~🟢~~ **Dropped** — CI/CD 범위, web-only | Medium | Variable |
...
## T-02: Cross-Model Review (Gate 2 Enhancement) — ~~Dropped~~ → Always-On Judge 흡수

> **상태**: Dropped — Always-On Judge (v1.0.0 JUDGE-01)에 흡수
> **대체 구현**: `codex-plugin-cc` (`/codex:adversarial-review`) 기반 Always-On advisory Judge
> **토론 기록**: `analysis/codex-plugin-judge-feasibility.md`

### 원안 (폐기)
gstack `/codex` — Independent code review with OpenAI Codex. 3 modes: gate (block), adversarial (challenge), consultation.

### 대체 설계: Always-On Judge (2026-04-01 합의)

T-02의 목적(cross-model 검증)은 Always-On Judge로 완전 대체:
- **매 Phase 후** `/codex:adversarial-review` 호출 (advisory, fire-and-forget)
- 비용: $0 (Google/OpenAI 기구독, 유휴 용량 활용)
- 별도 `mpl-cross-reviewer` 에이전트 불필요 — codex-plugin-cc가 대행
- API 키: `~/.codex/config.toml` (Codex CLI 설정 재사용)

### Open Questions (해소)
- [x] API key management? → Codex CLI의 `~/.codex/config.toml` 재사용
- [x] Default between OpenAI/Gemini? → Codex 1순위, Gemini 2순위 fallback
- [x] Token cost limit? → 불필요 (구독 내, 유휴 용량)
...
- [ ] ~~Intensity adjustment by maturity_mode?~~ **(v2: maturity_mode 제거됨. Gate 임계값 고정.)**
...
- [ ] Works in Light hat too? (cost is minimal since it's haiku) **(v2: Frugal tier → Light hat, pp_proximity 기반)**
...
## T-09: Performance Gate (Gate 1.5 Extension) — ~~Dropped~~

> **⚠ v2 상태**: Gate 1.5 제거됨 (v2에서 3H+1A Gate 구조). T-09는 Dropped.

### Inspiration
gstack `/benchmark` — Core Web Vitals baseline + bundle size regression detection.

### Current MPL State
- ~~Gate 1.5 has only coverage metrics~~ **(v2: Gate 1.5 제거)**
...
|---------|----------|:------:|
| v3.7 | Baseline: 2-Stage Interview, 15 agents, Adaptive Router, 5-Gate (v1) | ✅ |
...
### Recently Completed (v0.8.6~0.11.0)
...
| 0.11.2 | **Ambiguity Gate Enforcement**: Stage 1/2 분리, MCP 기반 모호성 채점 강제, PreToolUse+Stop 이중 게이트, decomposer 차단 hook | ✅ |
| 0.11.1 | **MCP Server Dependency Recovery**: mpl-setup에서 MCP 서버 dependency 누락 시 import 검증 + 자동 재설치 + 풀 리빌드 폴백 | ✅ |
| 0.11.0 | **v2 Phase 2 — Structural Transition**: Gate 3H+1A, Hat model (PP-proximity), 16→8 agents, Cluster Ralph removal, Redecomposition removal | ✅ |
| 0.12.1 | **v2 Completion** — 7 agent files deleted, v1→v2 terminology migration (38 files), routing-patterns backward compat, MCP dist rebuild, version history annotations | ✅ |
| 0.14.2 | **QMD Removal** — 4/11 제거 결정에 따라 17개 파일에서 QMD 참조 전부 제거 (doctor Category 11, setup Step 3g/7, qmd_verified 타입, qmd-verifier.mjs, qmd-setup.md 등). 런타임 영향 0. | ✅ |
| 0.14.1 | **Resume Workflow Data Integrity** (#35/#36/#37) — verbal pause state drift 복구 (paused_checkpoint enum + drift detection), keyword-detector slash-command guard, archivePreviousRun deep-copy로 .mpl/mpl/ subtree 보존 | ✅ |
...
| 0.16.0 | **3-Tier User Contract Architecture** — ygg-exp11 gap 3건(user-feature 포착 0, E2E 42/80 skip, phase-runner:test-agent = 83:1) 대응. Tier A' `.mpl/requirements/user-contract.md` + MCP tool `mpl_classify_feature_scope` + orchestrator Phase 0 Step 1.5 (0 agent 파일 신설). Tier B decomposer `covers:[UC-NN]` schema + `mpl-require-covers.mjs` hook. Tier C `@contract(UC-NN)` E2E coverage gate (strict 디폴트 + opt-out). MCP tool `mpl_diagnose_e2e_failure` (A/B/C/D 분류). Finalize Step 5.0.4 자동복구 loop + circuit breaker iter=2 + decomposer APPEND-MODE. Playwright trace 자동 수집. PP 불변성 guard hook `mpl-validate-pp-schema.mjs`. mpl-interviewer.md stale `mpl-ambiguity-resolver` ref 정리. | ✅ |
...
|---------|-------|----------|----------|
| ~~**v0.11.0**~~ | ~~**v2 Phase 2: 구조 전환**~~ | ~~GATE-01 (6→3H+1A) + HAT-01/02/03 (Hat+Floor) + AGT-02~05 (17→8 에이전트) + RETRY-01/02 + CLUST-01~~ | ✅ Done |
| **v1.0.0** | **v2 Phase 3: 완전체** | JUDGE-01/02/03 (Always-On Judge, codex-plugin-cc) + FLOW-01/01b/02/03 (Runner/Test 분리) + CB-L2 + DOC-05 | 🟠 Medium |
| **v1.0.1** | **Post-v2 경량 확장** | T-06 Doc Sync (Finalize 프롬프트 확장) | 🟡 Low |
| **v1.1.0** | **Post-v2 분석/관리** | T-08 Trend Retro + P-04 Skill Audit CLI (usage stats) | 🟡 Low |

### Dropped (2026-03-30 정리 — v2 대체 + post-v1.0.0 팀 합의)
...
| **T-02** Cross-Model Review | Always-On Judge에 흡수 (v1.0.0 JUDGE-01, codex-plugin-cc 기반) |
| **T-05** Design Contract | Seed contract_snippet으로 대체 가능. UI-only 빈도 낮음. 별도 에이전트 추가는 v2 축소 철학 위반 |
| **TS-01/02** MCP Assertion tools | Sentinel S0/S1/S3 + L1/L2 Diff Guard가 이미 기계적 spec 검증 수행. 외부 의존성 철학 위반 |
| **BM-04** Discovery 2축 매트릭스 | Hat+Floor 단일 축과 충돌. 분류가 Fix Loop 행동을 바꾸지 않음. 데이터 부재 |
| **F-269** RUNBOOK as docs | 채널 레지스트리 + DOC-05가 구조적 대체. 외부 스펙 추종, 실질 문제 부재 |
| **T-09** Performance Gate | 프로젝트별 편차(web-only). CI/CD 범위. 3 Hard Gate 철학과 긴장 |
| **F-06** Multi-Project | 실수요 미확인. YAGNI. 아키텍처 수준 변경(v2.0.0급) |
...
| **T-10** Ambiguity Gate Enforcement | ⚠️ Partial | ✅ **v0.11.0 done** (AGT-05) | Interviewer가 Ambiguity Resolver 흡수 |
...
| BM-03 | interface_contract weight 필드 (hard/soft/info) | ~~❌~~ **Dropped** — Hat Model PP-proximity 대체 (✅ v0.11.0 done) | ~~🟠 Medium~~ | ~~0.9.x~~ |
...
if violations_found:
  re-execute violated_phase (targeted fix)
```
...
else: mpl_failed()  # circuit break → pipeline failure
```
...
--- v2 Phase 2 ---
0.11.0: GATE-01 + HAT-01/02/03 + AGT-02~05 + RETRY-01/02 + CLUST-01 + MODEL-01 (구조 전환) (DONE)
0.11.1: MCP Server dependency recovery in mpl-setup (DONE)
0.11.2: Ambiguity Gate Enforcement — Stage 1/2 분리 + MCP scoring 강제 + decomposer 차단 hook (DONE)
0.14.2: QMD Removal — 17 files purged, 2 files deleted (qmd-verifier.mjs, qmd-setup.md), doctor 12→11 categories (DONE)
--- v2 Phase 3 ---
1.0.0: JUDGE-01/02/03 (Always-On Judge, codex-plugin-cc) + FLOW-01/01b/02/03 + CB-L2 + DOC-05 (완전체)
--- Post-v2 ---
1.0.1: T-06 Doc Sync (Finalize 프롬프트 확장)
1.1.0: T-08 Trend Retro + P-04 Skill Audit (usage stats)
--- Dropped (post-v1.0.0 팀 합의) ---
T-05, TS-01/02, BM-04, F-269, T-09, F-06
```
...
## Advisor Tool Integration Review & Contract Coverage (2026-04-10~11)

> **Source**: Anthropic Advisor Tool (beta `advisor-tool-2026-03-01`) 검토 + CB ablation 실험 데이터 결합 분석.
> 6개 구조화 debate (Pro/Con/Mutant 3-agent) + CB Phase A1 n=3+5 실험.
> Analysis: `wiki/scratch/2026-04-10/advisor-tool-mpl-integration-review.md`
> Decisions: `decision/2026-04-10-advisor-tool-*.md`, `decision/2026-04-11-cb-data-advisor-revisit.md`

### Debate 결과 요약

| Debate | 결론 |
|--------|------|
| S1: Phase Runner + Advisor | OLA — Opus를 Orchestrator 레벨에 집중 |
| S2: Advisory Gate → In-Flight | Observation Gate 재정의, post-hoc 유지, 피드백 루프 구축 |
| S3: Decomposer 경량화 | Opus 유지 — 계획은 투자 대상 |
| S4: Fix Loop Advisor | 내부 Advisor 부결, $0 배관 수리 |
| S5: Team-based Advisor | Team 패턴 부결, L1 Hook만 채택 |
| S6: CB 데이터 재검토 | Advisor 전원 부결, Contract coverage mandatory가 해법 |

**공통 결론**: Advisor API는 MPL에 도입하지 않음. 기존 메커니즘 강화가 우선.

### AD-01: Contract Coverage Mandatory (Decomposer Schema)

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| AD-01 | Contract Files Required in Decomposer | ⬜ Proposed | 🔴 High | TBD | ~0 | Enforcement |

**Problem**: Decomposer 스키마에 `contract_files` 필드가 없음(optional). Execute 레이어에서 `?.` optional chaining으로 skip. Hard 3에서 `contracts/` 없으면 auto-pass. 이 체인이 D2a shared omission이 Gate를 투명하게 통과하는 정확한 경로.

**CB 실험 근거**: C2=0/15(D2 미검출), C3=0/13(D3 미검출). Hard 3의 검사 능력은 충분하나 contract가 없어서 auto-pass.

**Proposal**:
1. `mpl-decomposer.md` 스키마에 `contract_files` required 추가
2. Decomposer가 impact 파일 간 boundary를 열거하고 각 boundary에 contract 생성
3. Execute 레이어의 optional chaining 제거

**관련**: CB-08(Mechanical Boundary Verification)의 L0 레이어 강화. CB-08은 contract가 존재할 때의 기계적 검증이고, AD-01은 contract 생성을 의무화.

### AD-02: Hard 3 Auto-Pass 제거

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| AD-02 | Hard 3 Auto-Pass Removal | ⬜ Proposed | 🔴 High | TBD | ~0 | Enforcement |

**Problem**: `mpl-run-execute-gates.md:174-176`에서 `contracts/` 디렉토리 미존재 시 Gate auto-pass. AD-01과 결합하면 contract가 항상 존재하므로 이 분기는 dead code가 되지만, 방어적으로 fail 전환.

**Proposal**: `"No contracts found. Skipping."` → `"No contracts found. FAIL."` 전환. AD-01 완료 후 적용.

### AD-03: Reflection Pattern Tag 기계적 수거

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| AD-03 | Reflection Tag Harvest (Step 5.2.6) | ⬜ Proposed | 🔴 High | TBD | ~0 | Learning |

**Problem**: Fix Loop의 `attempt-*.md` pattern tag가 Step 5.2.6의 학습 파이프라인에 흘러가지 않음. Reflection → learnings.md → semantic.md 경로에서 attempt 파일이 입력으로 포함되지 않음.

**Proposal**: Step 5.2.6에서 `Glob(".mpl/mpl/phases/*/reflections/attempt-*.md")` → pattern tag 기계적 추출 → `procedural.jsonl` 적재. tag는 이미 구조화된 enum (`type_mismatch`, `api_contract_violation` 등)이므로 LLM 불필요.

### AD-04: L1 Hook 확장 (PP 파일 감지)

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| AD-04 | PP-File Modification Sentinel Hook | ⬜ Proposed | 🟠 Medium | TBD | ~0 | Detection |

**Problem**: Phase Runner가 PP 참조 파일을 수정해도 기계적 감지가 없음. "모르는 걸 모르는" 문제의 L1 방어.

**Proposal**: PostToolUse hook on Edit|Write — 수정 파일 경로를 `pivot-points.md`의 파일 목록과 대조. 매칭 시 additionalContext 주입: "이 파일은 PP-N에서 참조됩니다. PP compliance를 확인하세요." 기존 sentinel(S0/S1/S3) 프레임워크 확장.

### AD-05: Hard 3 Parameter-Level 확장

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| AD-05 | Hard 3 Parameter-Level Verification | ⬜ Proposed | 🟡 Low | TBD | ~0 | Enforcement |

**Problem**: 현재 Hard 3는 key-set diff만 수행. "contract에 api_key가 있지만 구현에서 누락" = key는 있는데 실제 전달 안 함을 감지 못함. CB 실험 M2 37% leak의 원인.

**Proposal**: Hard 3의 L1 Diff Guard를 parameter value/type 수준으로 확장. Caller가 실제로 해당 parameter를 전달하는지 grep 기반 기계적 검증. AD-01(contract mandatory) 완료 후 적용.

**Note**: CB 실험의 37% leak은 M1=1(단일 워커) 조건. MPL Runner는 M1=0(별도 세션)이므로 직접 적용 여부 불확실. 우선순위 낮게 유지, 실측 후 결정.

### AD-06: Advisory Gate → Memory → Seed 피드백 루프

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| AD-06 | Advisory Report Feedback Loop | ⬜ Proposed | 🟠 Medium | TBD | ~0 (routing) | Learning |

**Problem**: Advisory Gate(Step 4.8)가 생성하는 `advisory-report.md`가 어디서도 소비되지 않음. Observation 데이터가 수집되지만 학습 루프에 연결 안 됨.

**Proposal**: advisory-report.md → Adaptive Memory F-25 semantic/procedural 레이어 경유 → 다음 run의 Phase 0 memory → Seed generation에 반영. Raw 주입 금지, memory 경유 압축 전달.

**관련**: S2 debate에서 Advisory Gate를 "Observation Gate"로 재정의한 결론의 구현. AD-06과 AD-03(pattern tag)이 동시에 Seed 생성 입력을 추가하므로 memory 경유 필수.

### AD-07: Fix Loop Strategy Gap 해소

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| AD-07 | Fix Loop Orchestrator Inline Strategy | ⬜ Proposed | 🟠 Medium | TBD | Opus cost | Strategy |

**Problem**: `mpl-state.mjs:370`에 하드코딩된 generic suggestion. checkConvergence가 "stagnating"을 감지하지만 "그 다음에 무엇을 해야 하는지"의 전략 생성이 빈 칸.

**Proposal**: Orchestrator가 stagnation 감지 시 prior reflection files + phase0 artifacts를 분석하여 구체적 전략(대안 접근법, must-not-do 리스트)을 생성하고 다음 retry의 Phase Seed에 주입. S1 OLA 결론의 구현.

### AD-08: Phase Runner Sonnet 통일 (단계적)

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| AD-08 | Phase Runner Sonnet Unification | ⬜ Proposed | 🟡 Low | TBD | 비용 절감 | Optimization |

**Problem**: PP-core/complex 페이즈에서 Opus Phase Runner 사용 → 비용 높음.

**Proposal**: AD-01+AD-02+AD-05로 Gate 강화 후, 단계적으로 Runner를 Sonnet으로 전환.
- Stage 1: Non-PP (AD-01+02 안정화 후)
- Stage 2: PP-adjacent (Stage 1 Hard 3 통과율 측정 후)
- Stage 3: PP-core, algorithm L/XL 예외 (Stage 2 측정 후)

각 Stage에서 Hard 3 통과율이 임계값 미만이면 해당 Stage에서 중단.

**전제**: AD-01, AD-02, AD-05 완료. "Gate를 먼저 강화하고, 그 다음에 Runner를 내려라."

### SP-0: Stage 2 specpill Socratic Loop Integration (브랜치 진행 중)

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| SP-0 | Stage 2 Socratic Loop + specpill Enhanced Path | 🟨 **In Progress** (feat/specpill-integration) | 🟠 Medium | TBD | ~0 | Integration |

**Branch**: `feat/specpill-integration` (commit `2ea6e64`)

**Already implemented on branch**:
- `docs/integrations/specpill.md` (+155): 전체 MPL-specpill 통합 계약
- `agents/mpl-ambiguity-resolver.md` (+854): Stage 2 enhanced path (specpill 유무에 따른 분기)
- `skills/mpl-setup/references/specpill-setup.md` (+94): 설치 가이드
- `agents/mpl-doctor.md` (+18): Category 13 (WARN 캡, specpill은 optional)
- `commands/mpl-run-phase0.md` (+2): Step 0 probe 연동
- `.claude-plugin/plugin.json`, `marketplace.json`: 플러그인 등록

**Design**:
```
Step 0 (Triage): doctor probe → session.specpill_available 플래그
Step 1 Stage 1 (interviewer): specpill 호출 없음
Step 1 Stage 2 (ambiguity-resolver):
  if specpill_available:
    init_session → add_feature/flow_node/ui_element →
    wait_for_feedback(1500ms) pre-empt → AskUserQuestion fallback →
    ambiguity ≤ 0.2 AND 사용자 승인 → finalize_spec(approved_by="user")
  else:
    text-only Socratic loop (현행 유지, 완전 fallback)
Step 2+: spec.json on-disk reference, 추가 호출 없음
```

**핵심 설계 원칙**:
- specpill은 **correctness가 아닌 clarity layer** — `mpl_score_ambiguity`가 여전히 termination 기준
- **완전 optional** — WARN-not-FAIL (doctor Category 13)
- **destructive tool 사용 안 함** (`reset_session`, `delete_feature` 제외)
- **버전 핀 없음** — tool surface만 호환되면 OK

**Branch 상태 경고**: 브랜치의 `docs/roadmap/pending-features.md`가 2026-03-31 기준으로 stale. pre-v2 agent 참조(`mpl-compound`, `mpl-scout`, `mpl-code-reviewer`, `mpl-verification-planner`) 포함. **main의 v2 cleanup 기준으로 rebase 필요**.

**TODO (머지 전)**:
- [ ] main rebase + v2 agent 참조 정리
- [ ] Step 0 probe 메커니즘 실측 (check_tool_available 호출 패턴)
- [ ] PoC: specpill 미설치 환경에서 fallback 동작 검증
- [ ] PoC: specpill 설치 환경에서 enhanced path end-to-end 검증

### SP-01: specpill Spec → Boundary Contract 자동 파생 (신규, Step 2+)

| ID | Feature | Status | Priority | Version | Token Cost | Type |
|----|---------|--------|----------|---------|------------|------|
| SP-01 | Spec-to-Contract Auto Derivation | ⬜ Proposed | 🟡 Low | TBD | ~0 | Prevention |

**Problem**: SP-0은 Phase 0의 Stage 2에서만 specpill을 사용하고, "Step 2 onward: spec.json remains on disk as a reference artifact; no further specpill calls"로 명시됨. 즉 `spec.json`의 구조화된 데이터가 Decomposer의 contract 생성에 활용되지 않음.

AD-01(contract mandatory)은 Decomposer가 contract를 수동 생성하는 방식. specpill의 구조화된 출력(Feature × Flow × UI × Link)에서 Phase-level boundary contract를 자동 파생하면 수동 부담 제거 + D2a shared omission 구조적 방지.

**Proposal**: Step 2 (Decomposer) 확장 — `session.specpill_available && .specpill/spec.json 존재` 시 spec.json을 읽어 Feature의 flow graph + UI link 구조에서 Phase 간 boundary를 자동 열거 → `contract_files` 자동 생성.

```
specpill spec (from .specpill/spec.json):
  Feature "login"
    FlowNode: validate_credentials → check_2fa
    UIElement: submit_btn ──triggers──→ validate_credentials
    UIElement: error_toast ──renders_from──→ validate_credentials
→ Decomposer 도출:
  Phase 1 (backend): produces { validateCredentials(email, password) → Result }
  Phase 2 (frontend): requires { validateCredentials } + renders { error_toast }
  → contract_files 자동 생성
```

**관계**:
- AD-01(contract mandatory)의 **구현 경로 업그레이드** — 수동 생성 → 자동 파생
- SP-0(Stage 2 integration)의 **하류 확장** — Phase 0에서 수집한 spec을 Step 2에서 재활용
- specpill 없는 환경에서는 AD-01의 수동 contract가 fallback

**전제**: SP-0 머지 + AD-01 완료. SP-01은 두 feature의 상위 통합 레이어.

### 적용 순서 (충돌 해소 반영)

```
Phase 1 (즉시, 병렬, $0):
  AD-01 (contract mandatory) ─┬──→ Phase 2
  AD-02 (auto-pass 제거)     ─┘
  AD-03 (pattern tag 수거)   ──→ (독립)
  AD-04 (L1 Hook 확장)      ──→ (독립)

Phase 2 (AD-01/02 안정화 후):
  AD-05 (Hard 3 parameter 확장)  ─┬──→ Phase 3
  AD-06 (Advisory→Memory→Seed)   ─┘
  AD-07 (Fix Loop strategy gap)  ──→ (독립)

Phase 3 (Gate 강화 완료 후, 단계적):
  AD-08a (Runner Sonnet — Non-PP)
  AD-08b (Runner Sonnet — PP-adjacent) ← 08a 측정 후
  AD-08c (Runner Sonnet — PP-core)     ← 08b 측정 후

specpill 연동:
  SP-0 (Stage 2 Socratic Loop) ← feat/specpill-integration 브랜치 진행 중, main rebase 필요
  SP-01 (Spec→Contract 자동 파생) ← SP-0 머지 + AD-01 완료 후
```
...
| 후보 | 기각 이유 | Debate |
|------|----------|--------|
| Advisor API 도입 (Runner 내부) | MPL 아키텍처와 구조적 불일치. fresh session transcript 빈곤, tool 불가 | S1 |
| Advisory Gate In-Flight 전환 | Observation Gate로 재정의, post-hoc이 본질 | S2 |
| Decomposer Sonnet + Advisor | 계획은 투자 대상, blast radius, 1회 실행 비용 미미 | S3 |
| Fix Loop 내부 Advisor | F-27+수렴 검출 충분, tool 불가로 맹인 진단 | S4 |
| Team SendMessage Consultable Orchestrator | 복잡성/latency/병목 비용 초과, "잘못 분해된 Phase" | S5 |
| Advisor로 L2/L3 방어 | Contract coverage gap이 진짜 원인, Advisor 증거 없음 | S6 |
