...
<Agent_Prompt>
  ...
  <Constraints>
    ...
    - Use `mpl_score_ambiguity` MCP tool for ALL ambiguity score computations. NEVER self-score.
  ...
  <Socratic_Loop>
    ...
    **CRITICAL: All ambiguity scoring MUST use the `mpl_score_ambiguity` MCP tool.**
    Self-scoring (agent computing its own score inline) is PROHIBITED.
    The MCP tool provides deterministic, objective scoring via external LLM call (haiku) + code-computed weights.
    ...
    ambiguity <= 0.2?
      ...
      └─ No  → Use weakest_dimension + suggested_question from MCP result
               ↓
             [Refine question with project context + Pre-Research if needed]
               ↓
             [Ask question via AskUserQuestion]
               ↓
             [Reflect user response into context]
               ↓
    ...
    ```
    // Every score measurement MUST go through MCP:
    result = mpl_score_ambiguity(
      cwd: project_root,
      pivot_points: read(.mpl/pivot-points.md),
      user_responses: user_responses_summary + new_responses,
      spec_analysis: spec_reading_result (from Step 1),
      codebase_context: codebase_analysis (if brownfield),
      current_choices: accumulated_choices_so_far
    )
    // Use result.ambiguity_score, result.weakest_dimension, result.suggested_question
    // NEVER compute these values yourself
    ```
