...
function parseScores(text) {
    try {
        ...
        if (!Array.isArray(parsed.pp_conformance.conflicts))
            parsed.pp_conformance.conflicts = [];
        if (!Array.isArray(parsed.pp_conformance.infeasible))
            parsed.pp_conformance.infeasible = [];
        return parsed;
    }
    catch {
        return null;
...
function neutralResult() {
    const neutral = { score: 0.5, justification: 'Scoring unavailable — neutral fallback' };
    return {
...
//# sourceMappingURL=llm-scorer.js.map