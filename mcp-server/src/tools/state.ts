...
export async function handleStateRead(args: { cwd: string; keys?: string[] }) {
  const state = readState(args.cwd);
  if (!state) {
    return { content: [{ type: 'text' as const, text: JSON.stringify({ state: null, error: 'No MPL state found. Is MPL active?' }) }] };
  }
  const filtered = args.keys?.length ? filterState(state, args.keys) : state;
  return { content: [{ type: 'text' as const, text: JSON.stringify({ state: filtered }) }] };
}
