...
If enabled:
```
// IMPORTANT: Use CLAUDE_PLUGIN_ROOT variable (not hardcoded version path!)
// CLAUDE_PLUGIN_ROOT is resolved at runtime by Claude Code to the current plugin cache path.
// This ensures HUD survives version upgrades without manual path updates.

// Configure in ~/.claude/settings.json
settings.statusLine = {
  "type": "command",
  "command": "node \"${CLAUDE_PLUGIN_ROOT}/hooks/mpl-hud.mjs\""
}
```

**CRITICAL**: Do NOT use hardcoded cache paths like `~/.claude/plugins/cache/mpl/mpl/X.Y.Z/...`.
The `${CLAUDE_PLUGIN_ROOT}` variable auto-resolves to the correct versioned path at runtime.
Hardcoded paths break when the plugin version is bumped.
